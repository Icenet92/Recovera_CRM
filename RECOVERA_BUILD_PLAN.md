# RECOVERA — Phased Build Plan

## How to use this document

**First, give whichever AI coding tool you're using the separate `RECOVERA_RULES.md` file** — those are the standing rules that apply across every phase, regardless of tool. This document is the feature-by-feature phase content only.

Do **not** paste this whole file into your coding tool at once. Paste **one phase at a time**, in order. After each phase, open the app, click through what was built, and confirm it actually works before you move to the next phase. If a phase doesn't compile or a feature is faked, stop and fix it before moving on — don't let the next phase build on top of a broken one.

Each phase below is written so you can copy it directly as a prompt. Re-share `RECOVERA_RULES.md` (or paste Phase 0 below) at the start of every phase, since most tools won't retain earlier sessions' context on their own.
...
Steps for whichever tool is building this to follow, in order:
...
**Before moving to Phase 3B:** confirm you can create a case with a debtor, assign it to an employee, see the outstanding amount compute correctly (with 0 payments, outstanding = total claim), reassign it and see that logged, and close/reopen a case with the audit trail intact.

---

## PHASE 3B — Recovery Assignments (Case Pools) and Time-Based Measurement

**Goal:** the boss's actual way of setting and measuring performance — a manager pools a set of cases, hands the whole batch to one recovery agent with a target amount and a deadline, and results get measured against that specific batch: how much was recovered, and how long it took. This is a separate concept from Phase 3's single-case ownership (primary owner/supervisor/supporting employees) — it sits alongside it, not instead of it. A case still has one accountable owner; a batch is the goal-setting wrapper multiple of that owner's cases can belong to.

1. Create tables: `recovery_assignments` (id, assigned_employee_id, assigned_by, target_amount, start_date, deadline_date, status [Active/Completed/Overdue/Cancelled], notes) and `case_assignment_batch_history` (id, recovery_assignment_id, case_id, added_date, removed_date — nullable) so a case's batch membership over time is preserved, not overwritten.
2. Build the "Create Recovery Assignment" flow for managers: select an employee, select which cases to pool (must all belong to that employee's assigned cases, or be assigned to them as part of this action), auto-suggest a target amount as the sum of outstanding claims across the selected cases (manager can override this number), set a deadline date, save.
3. Do not require every case to belong to a batch — a case can exist and be worked without ever being pooled. Batches are an additional layer for goal-setting, not a mandatory wrapper.
4. Time-to-recovery must be computed, not manually tracked: for a case, this is (the timestamp it reached a "resolved"-type status, from Phase 3's `case_status_history`) minus (the timestamp it was added to its recovery assignment, from `case_assignment_batch_history`). If a case isn't in a batch, fall back to (resolved timestamp − case opened date).
5. Build a "Recovery Assignment" detail view: target amount vs. actual recovered so far (sum of verified payments across the batch's cases — this will read 0 until Phase 5 exists, and that's correct, not a bug), deadline vs. today (time remaining, or overdue by how much), and a list of the pooled cases with each one's individual status and time-in-progress.
6. This does NOT replace or change the daily activity/task logging from Phase 4 — a recovery agent's day-to-day calls, notes, and tasks work exactly the same whether or not their cases happen to be in an active batch.

**Before moving to Phase 4:** as a manager, create a batch of 2-3 test cases assigned to one Recovery Officer, confirm the target amount auto-suggests correctly and can be overridden, set a deadline, and confirm the batch detail view shows the pooled cases and a correct (currently zero, since no payments exist yet) recovered amount.
...
3. Every quick action on a case (Call Debtor, Add Note, etc.) must write an `activities` row automatically with employee, timestamp, related case, description, outcome. Do not require a separate manual "log this activity" step after the action — the action itself is the log entry. For call activities specifically, `outcome` must be a defined set of values, not freeform text: Reached/Answered, No Answer, Voicemail, Wrong Number, Callback Requested, Declined — this is what makes the escalation rule in step 4 possible to compute.
4. Build task auto-creation rules: new case → review/contact-debtor/review-documents tasks; deadline approaching → escalation task; **3 consecutive call activities on the same case with a no-contact outcome (No Answer, Voicemail, or Wrong Number) → automatically flag the case as "Needs Attention" (visible as a badge/highlight on the case) and create an escalation task suggesting a meeting be scheduled instead of another call attempt.** Keep this list short for v1; don't build a generic rules engine yet.
...
14. Add an **Assignment Performance view**, separate from the periodic (weekly/monthly) KPI table above — this is what the boss actually asked for as the primary way to measure agents: per Recovery Assignment (from Phase 3B), show target amount vs. actual recovered, deadline vs. time elapsed/remaining, and each pooled case's individual time-to-recovery. Also compute a per-employee **average time-to-recovery** across their resolved cases (regardless of batch) as its own trend-able number, separate from the weighted weekly score — this is a genuinely different question ("how fast do they close cases") from "did they hit this week's targets," and the boss should be able to see both without one being buried inside the other.
15. The periodic KPI table (steps 1-13) and the Assignment Performance view (step 14) both stay — they answer different questions and neither replaces the other. Show both on the Employee 360° page, in clearly separate sections.
...
## Notes for you (not part of the build prompt)
...
- If the coding tool produces something inconsistent with the Master/Worker LAN pattern (e.g. tries to build a cloud backend instead), stop and correct it before continuing — that architectural choice needs to hold across every later phase, the same way it does in HarvestSync.
